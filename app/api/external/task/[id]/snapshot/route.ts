...
export async function GET(
  ...
  try {
    ...
    const upstream = await proxyJsonToExternalApi({
      requestId,
      path: `/task/${encodeURIComponent(id)}/snapshot`,
      query: {
        include_items: searchParams.get("include_items") || undefined,
    ...
    return internalErrorResponse(requestId, "api/external/task/[id]/snapshot", error)
  }
}
